  ...
  function triggerNotificacionTareaAsignada($conn, $id_tarea, $id_usuario_asignado, $id_usuario_anterior = null, $es_libre = 0) {
      ...
      try {
          // Obtener información de la tarea y proyecto (incluyendo flag es_libre del proyecto)
          $stmt = $conn->prepare("
              SELECT t.nombre as nombre_tarea, p.nombre as nombre_proyecto, p.es_libre as proyecto_es_libre
              FROM tbl_tareas t
          ...
          // Si no se pasó el flag explícitamente, derivarlo del proyecto
          $flag_libre = (int)$es_libre === 1 ? 1 : (int)($row['proyecto_es_libre'] ?? 0);
  ...
  function triggerNotificacionProyectoAsignado($conn, $id_proyecto, $id_usuario_asignado, $id_usuario_anterior = null, $es_libre = 0) {
      // Si es el mismo usuario, no notificar
      if ($id_usuario_anterior !== null && (int)$id_usuario_anterior === (int)$id_usuario_asignado) {
          return false;
      }
      ...
      try {
          // Obtener nombre del proyecto + es_libre de la BD como verificación
          $stmt = $conn->prepare("SELECT nombre, es_libre FROM tbl_proyectos WHERE id_proyecto = ?");
          ...
          // Usar el flag del parámetro si se pasó, sino el de la BD (para llamadas legacy)
          $flag_libre = (int)$es_libre === 1 ? 1 : (int)($row['es_libre'] ?? 0);
  ...
  function triggerNotificacionProyectoGrupal($conn, $id_proyecto, $id_usuario, $es_libre = 0) {
      try {
          $stmt = $conn->prepare("SELECT nombre, es_libre FROM tbl_proyectos WHERE id_proyecto = ?");
          $stmt->bind_param("i", $id_proyecto);
          $stmt->execute();
  ...
  function triggerNotificacionProyectoVencido($conn, $id_proyecto, $estado_anterior) {
      ...
      try {
          // Obtener información del proyecto (incluye es_libre)
          $stmt = $conn->prepare("SELECT nombre, id_creador, id_participante, es_libre FROM tbl_proyectos WHERE id_proyecto = ?");
          $stmt->bind_param("i", $id_proyecto);
          $stmt->execute();
          $result = $stmt->get_result();
          ...
          $flag_libre = (int)($proyecto['es_libre'] ?? 0);
