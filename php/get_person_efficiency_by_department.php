...
try {
  ...
  if (!$dept_stmt) {
    throw new Exception(
      "Error preparando consulta de departamento: " . $conn->error,
    );
  }
  ...
  $query = " 
        ...
        GROUP BY u.id_usuario, u.nombre, u.apellido, u.num_empleado, ur.id_rol, r.nombre 
  ...
  echo json_encode($response, JSON_UNESCAPED_UNICODE);
  error_log(
    "Error en get_person_efficiency_by_department.php: " . $e->getMessage(),
  );
}
...
function processPersonEfficiencyData($efficiency_data)
{
  ...
  return [
    ...
    "details" => $details,
    "max_tasks" => max(
      array_map(fn($d) => $d["total_tareas"], $efficiency_data),
    ),

    "avg_completion" => round(
      array_sum(
        array_map(fn($d) => $d["tasa_completacion"], $efficiency_data),
      ) / count($efficiency_data),
...
function getOfficialDashboardColors($count)
{
  ...
  $colors = [
    // Verde primario
    [
      "border" => "rgba(34, 139, 89, 1)",
      "background" => "rgba(34, 139, 89, 0.7)",
    ],

    // Verde claro
    [
      "border" => "rgba(80, 154, 108, 1)",
      "background" => "rgba(80, 154, 108, 0.7)",
    ],

    // Verde oscuro
    [
      "border" => "rgba(24, 97, 62, 1)",
      "background" => "rgba(24, 97, 62, 0.7)",
    ],

    // Ice/Gris claro
    [
      "border" => "rgba(200, 205, 210, 1)",
      "background" => "rgba(200, 205, 210, 0.7)",
    ],

    // Gris
    [
      "border" => "rgba(130, 140, 150, 1)",
    ...
    // Verde secundario
    [
      "border" => "rgba(45, 110, 80, 1)",
      "background" => "rgba(45, 110, 80, 0.7)",
    ],

    // Gris claro
    [
      "border" => "rgba(160, 170, 180, 1)",
      "background" => "rgba(160, 170, 180, 0.7)",
    ],

    // Variaciones adicionales con opacidad diferente
    [
      "border" => "rgba(34, 139, 89, 0.9)",
      "background" => "rgba(34, 139, 89, 0.6)",
    ],

    [
      "border" => "rgba(80, 154, 108, 0.9)",
      "background" => "rgba(80, 154, 108, 0.6)",
    ],

    [
      "border" => "rgba(24, 97, 62, 0.9)",
      "background" => "rgba(24, 97, 62, 0.6)",
    ],

    [
      "border" => "rgba(45, 110, 80, 0.9)",
      "background" => "rgba(45, 110, 80, 0.6)",
    ],

    [
      "border" => "rgba(200, 205, 210, 0.9)",
      "background" => "rgba(200, 205, 210, 0.6)",
    ],

    [
      "border" => "rgba(130, 140, 150, 0.9)",
