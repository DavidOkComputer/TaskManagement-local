...
try {
    ...
    // Si el cliente envio la opcion sintetica "Supervisor" (id_rol = -2),
    // la mapeamos al rol real de gerente (id_rol = 2) y marcamos es_supervisor = 1.
    // El cliente tambien puede enviar es_supervisor explicitamente.
    $es_supervisor = isset($_POST['es_supervisor']) ? intval($_POST['es_supervisor']) : 0;
    ...
    // Un supervisor solo tiene sentido si es gerente (id_rol = 2)
    if ($es_supervisor === 1 && $id_rol !== 2) {
        throw new Exception('Solo un gerente puede ser marcado como supervisor');
    }
    ...
    try {
        ...
        // Insertar usuario en tbl_usuarios (ahora incluye es_supervisor)
        $stmt = $conn->prepare("
            INSERT INTO tbl_usuarios
                (nombre, apellido, usuario, num_empleado, acceso,
                 id_departamento, id_rol, id_superior, es_supervisor, e_mail, foto_perfil)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        if (!$stmt) {
        ...
        $response['message']       = "Usuario '{$usuario}' creado exitosamente"
            . ($es_supervisor === 1 ? ' (Supervisor)' : '');
        $response['id_usuario']    = $nuevo_id;
        $response['usuario']       = $usuario;
        $response['foto_perfil']   = $foto_perfil;
        $response['es_supervisor'] = (bool)$es_supervisor;
        ...
        error_log("Usuario creado: ID={$nuevo_id}, Usuario={$usuario}, Num Empleado={$num_empleado}, Rol={$id_rol}, Depto={$id_departamento}, Supervisor={$es_supervisor}");
