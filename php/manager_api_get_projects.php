...
try {
    if ($is_admin) {
        ...
        if (!$stmt) {
            throw new Exception('Error preparando consulta admin: ' . $conexion->error);
        }

    } else {
        // Construir IN clause con todos los departamentos gestionados
        $dept_ids_for_query = !empty($departamentos_gerente) ? $departamentos_gerente : [$departamento_principal];
        $placeholders_dept = implode(',', array_fill(0, count($dept_ids_for_query), '?'));
        ...
        if (!empty($subordinados_ids)) {
            ...
            $query = "
                ...
                WHERE (
                    p.id_departamento IN ($placeholders_dept)
                    OR p.id_creador = ?
                    OR p.id_participante = ?
                    OR pu.id_usuario = ?
                    OR (p.es_libre = 1 AND p.id_creador = ?)
            ...
            // dept_ids + 4 base params (creador, participante, pu.usuario, es_libre+creador) + 3x subordinados
            $types = str_repeat('i', count($dept_ids_for_query))
                . 'iiii'
                . str_repeat('i', count($subordinados_ids) * 3);
            ...
            // dept_ids + 4 base params (creador, participante, pu.usuario, es_libre+creador)
            $types = str_repeat('i', count($dept_ids_for_query)) . 'iiii';
            $params = array_merge(
                $dept_ids_for_query,
                [$id_usuario, $id_usuario, $id_usuario, $id_usuario]
            );
    ...
    while ($proyecto = $result->fetch_assoc()) {
        ...
        $proyectos[] = [
            ...
            'puede_editar_otros'    => (int)($proyecto['puede_editar_otros'] ?? 0),
