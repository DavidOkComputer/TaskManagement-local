...
<?php
/*save_task.php para crear tareas con validacion de permisos de asignacion y membresía del proyecto */
ob_start();
...
$notification_helper_exists = file_exists("../email/NotificationHelper.php");
...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
	ob_clean(); //limpiar cualquier output antes de mandar el json
	echo json_encode([
		"success" => false,
...
try {
	...
	$fecha_vencimiento = isset($_POST["fecha_vencimiento"])
	...
	$id_creador = isset($_POST["id_creador"])
		? intval($_POST["id_creador"])
		: 0;
	...
	$stmt = $conn->prepare("SELECT id_creador, 
                                          puede_editar_otros, 
                                          fecha_inicio, 
                                          id_tipo_proyecto, 
                                          id_participante  
	...
	if ($id_participante !== null && $id_participante != $id_creador) {
		$notif_result = triggerNotificacionTareaAsignada(
			$conn,
			$task_id,
			$id_participante,
		);
		...
		if (
			$notif_result &&
			isset($notif_result["success"]) &&
			$notif_result["success"]
		) {
			error_log(
				"Notificación de tarea asignada enviada - Tarea: $task_id, Usuario: $id_participante",
			);
		}
	...
	//limpiar todo el output antes de enviar al json
	ob_clean();
	echo json_encode([
		"success" => false,
...
function recalculateProjectProgress($conn, $id_proyecto)
{
	try {
		...
		error_log(
			"Error recalculando progreso del proyecto: " . $e->getMessage(),
		);
	}
}
...
function determineProjectStatus($progress, $id_proyecto, $conn)
{
	try {
		...
		error_log(
			"Error determinando estado del proyecto: " . $e->getMessage(),
		);
