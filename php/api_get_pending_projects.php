...
/*api_get_pending_projects.php para saber TODOS los proyectos pendientes*/
ob_start();
session_start();
...
try {
  // Obtener TODOS los proyectos pendientes sin restricciones de usuario
  $query = " 
        SELECT DISTINCT  
            p.id_proyecto, 
            p.nombre, 
  ...
  while ($proyecto = $result->fetch_assoc()) {
    ...
    $proyectos[] = [
      ...
      "departamento" => htmlspecialchars(
        $proyecto["nombre_departamento"] ?? "N/A",
      ),
