...
/*update_task.php para actualizar tareas*/
...
try {
  ...
  $id_proyecto = isset($_POST["id_proyecto"])
    ? intval($_POST["id_proyecto"])
    : 0;

  $fecha_cumplimiento = isset($_POST["fecha_vencimiento"])
  ...
  $id_participante =
    isset($_POST["id_participante"]) && !empty($_POST["id_participante"])
  ...
  $stmt = $conn->prepare(
    "SELECT id_proyecto, id_participante FROM tbl_tareas WHERE id_tarea = ?",
  );
  ...
  ); //verificar que el proyecto existe y obtener permisos, tipo y fecha de inicio
  ...
  if ($projectData["puede_editar_otros"] == 0) {
    if ($projectData["id_creador"] != $id_creador) {
      throw new Exception(
        "Solo el creador del proyecto puede asignar tareas a este proyecto",
      );
    }
  }
  ...
  if ($id_participante !== null) {
    ...
    if ($projectData["id_tipo_proyecto"] == 1) {
      ...
      $stmt = $conn->prepare(
        "SELECT id_usuario FROM tbl_proyecto_usuarios WHERE id_proyecto = ? AND id_usuario = ?",
      );
      ...
      if ($result->num_rows === 0) {
        throw new Exception(
          "El usuario especificado no está asignado a este proyecto grupal",
        );
      }
    ...
    elseif ($projectData["id_tipo_proyecto"] == 2) {
      ...
      if ((int) $projectData["id_participante"] !== $id_participante) {
        throw new Exception(
          "El usuario especificado no está asignado a este proyecto individual",
        );
      }
    }
  }
  ...
  if ($id_participante !== null && $id_participante != $old_id_participante) {
    triggerNotificacionTareaAsignada(
      $conn,
      $id_tarea,
      $id_participante,
      $old_id_participante,
    ...
    error_log(
      "Notificación enviada: Tarea {$id_tarea} asignada a usuario {$id_participante}",
    );
    ...
    try {
      ...
      error_log(
        "Error enviando correo de asignación: " . $email_error->getMessage(),
      );

      // No lanzar excepción, continuar con la actualización exitosa
    }
  }
  ...
  if (
    $estado_anterior_proyecto !== "vencido" &&
    $estado_nuevo_proyecto === "vencido"
  ) {
    triggerNotificacionProyectoVencido(
      $conn,
      $old_id_proyecto,
      $estado_anterior_proyecto,
    );
  }
...
{
  $stmt = $conn->prepare(
    "SELECT estado FROM tbl_proyectos WHERE id_proyecto = ?",
  );
...
function recalculateProjectProgress($conn, $id_proyecto)
{
  try {
    ...
    $stmt = $conn->prepare(
      "SELECT COUNT(*) as total FROM tbl_tareas WHERE id_proyecto = ?",
    );
    ...
    $stmt = $conn->prepare(
      "UPDATE tbl_proyectos SET progreso = ?, estado = ? WHERE id_proyecto = ?",
    );
    ...
    error_log(
      "Progreso del proyecto $id_proyecto actualizado a: $progress% - Estado: $nuevo_estado",
    );
  } catch (Exception $e) {
...
function determineProjectStatus($progress, $id_proyecto, $conn)
{
  try {
    ...
    $stmt = $conn->prepare(
      "SELECT fecha_cumplimiento FROM tbl_proyectos WHERE id_proyecto = ?",
    );
