 /*manage_users_gerente.js para mostrar los empleados del departamento del gerente*/ 
 const Config = {
 	API_ENDPOINTS: {
 ...
 const AUTO_REFRESH_CONFIG = {
 	USERS_INTERVAL: 60000,
 	MODAL_INTERVAL: 60000,
 	DEBUG: true
 };
 ...
 function setupModalEventListeners() { //listener de abrir o cerrrar modal 
 ...
 function startAutoRefresh() {
 	...
 	autoRefreshInterval = setInterval(() => { //actualizar info y progreso de usuario 
 		if (AUTO_REFRESH_CONFIG.DEBUG) {}
 		refreshUserData();
 	}, AUTO_REFRESH_CONFIG.USERS_INTERVAL);
 	startModalAutoRefresh(); //intervalo separado para el modal de proyectos cuando se abre 
 	if (AUTO_REFRESH_CONFIG.DEBUG) {}
 }
 ...
 function startModalAutoRefresh() {
 	...
 	modalRefreshInterval = setInterval(() => { //revisar si esta abierto el modal y actualizarlo 
 		const modal = document.getElementById('viewProjectsModal');
 ...
 async function fetchUserProjects(userId) {
 	try {
 		const response = await fetch(`../php/manager_get_user_projects.php?id_usuario=${userId}`);
 ...
 async function showUserProjects(userId, userName, userEmail) {
 	...
 	// establecer usuario para el autorefresh 
 ...
 function getProfilePictureHTML(usuario, nombreCompleto) {
 	const initials = getInitials(nombreCompleto);
 	const bgColor = getAvatarColor(usuario.id_usuario);
 	//revisar si el usuario tine una foto
 	if (usuario.foto_url) {
 		//usar imagen pequenia si esta disponible, sino usar imagen grande
 		const imageUrl = usuario.foto_thumbnail || usuario.foto_url;
 		return ` 
            <div class="profile-avatar me-3" style="position: relative;"> 
                <img src="../${imageUrl}"  
        ...
        `;
 		...
 		//mostrar iniciales si ocrre algun error 
 		return ` 
            <div class="profile-avatar me-3"> 
                ${getInitialsAvatarHTML(initials, bgColor)} 
            </div> 
        `;
 	}
 }
 ...
 function getInitialsAvatarHTML(initials, bgColor) {
 	return `<div class="rounded-circle d-flex align-items-center justify-content-center"  
                 style="width: 45px; height: 45px; background-color: ${bgColor}; color: white; font-weight: 600; font-size: 16px; border: 2px solid #e9ecef;"> 
                ${initials} 
            </div>`;
 }
 ...
 async function displayUsuarios(usuarios) {
 	...
 	//calcular el progreso para todos los usuarios paginados 
 ...
 function createUsuarioRow(usuario) {
 	...
 	const profilePictureHTML = getProfilePictureHTML(usuario, nombreCompleto);
 	tr.innerHTML = ` 
 ...
 function filterUsuarios() {
 	...
 	if (!searchInput.trim()) {
 		filteredUsuarios = [...allUsuarios];
 		currentPage = 1;
 		const sorted = currentSortColumn ? sortUsuarios(filteredUsuarios, currentSortColumn, sortDirection) : filteredUsuarios;
 		displayUsuarios(sorted);
 		return;
 	}
 ...
 function validateEmail(email) {
 	...
 	if (!emailRegex.test(email)) {
 		return {
 			isValid: false,
 			message: 'Formato de email inválido (ej: usuario@ejemplo.com)'
 		};
 	}
 	return {
 		isValid: true,
 ...
 function validateTextField(value, fieldName = 'Campo', minLength = 1) {
 	...
 	if (value.trim().length > 100) {
 		return {
 			isValid: false,
 			message: `${fieldName} no puede exceder 100 caracteres`
 		};
 	}
 	return {
 		isValid: true,
