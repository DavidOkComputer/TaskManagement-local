...
function updateUserDashboardStats(stats) {
  ...
  const tareasCompletadasEl = document.getElementById(
    "stat-tareas-completadas",
  );
  if (tareasCompletadasEl) {
    tareasCompletadasEl.textContent =
      (stats.porcentaje_tareas_completadas || 0) + "%";
  }
    ...
    datasets: [
      {
        data: data,
        backgroundColor:
          total === 0 ? ["#F2994A"] : ["#F2C94C", "#009b4a", "#C62828"],
        borderWidth: 2,
        borderColor: "#fff",
      },
    ],
...
function displayMyProjectsProgress(proyectos) {
  ...
  proyectos.forEach((proyecto) => {
    ...
    //insignia extra para proyectos libres
    const libreBadge = proyecto.es_libre
      ? '<span class="badge badge-info ms-1" ' +
        'title="Proyecto multidepartamental">' +
        "Libre</span>"
      : "";
    ...
    row.innerHTML = `
            <td>
                <strong>
                    ${escapeHtml(truncateText(proyecto.nombre, 20))}
                </strong>
                ${libreBadge}
                <br>
            ...
            <td>
                ...
                <small class="text-muted d-block mt-1">
                    ${proyecto.mis_tareas_completadas}/
                    ${proyecto.mis_tareas_total} tareas
                </small>
            </td>
        `;
