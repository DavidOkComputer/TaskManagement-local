...
function displayProjectDetails(proyecto) {
    ...
    progressBar.className = `progress-bar ${getProgressColorClass(progreso)}`;
    ...
    displayProjectObjectives(proyecto.objetivos);
...
function displayProjectObjectives(objetivos) {
    const container = document.getElementById('detailObjetivosList');
    const countElement = document.getElementById('detailObjetivosCount');
    
    countElement.textContent = objetivos.length;
 
    if (!objetivos || objetivos.length === 0) {
        container.innerHTML = '<p class="text-muted text-center mb-0">Sin objetivos registrados</p>';
        return;
    }
 
    let html = '';
    objetivos.forEach(objetivo => {
        const estadoClass = objetivo.estado.toLowerCase().replace(' ', '-');
        html += `
            <div class="objetivo-item ${estadoClass}">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <strong>${escapeHtml(objetivo.nombre)}</strong>
                        <small class="text-muted d-block">${formatDateShort(objetivo.fecha_cumplimiento)}</small>
                    </div>
                    <span class="badge badge-${getStatusColor(objetivo.estado)}">${capitalizeFirst(objetivo.estado)}</span>
                </div>
            </div>
        `;
    });
...
function displayProjectUsers(usuarios) {
    ...
    usuarios.forEach(usuario => {
        const progressClass = getProgressColorClass(usuario.progreso);
        html += `
            <tr>
                <td>
...
function displayProjectTasks(tareas, filter = 'all') {
    ...
    tareasToShow.forEach(tarea => {
        const prioridadClass = getPriorityBadgeClass(tarea.prioridad);
        ...
        html += `
            <tr>
                ...
                <td>
                    <span class="badge ${prioridadClass}">${capitalizeFirst(tarea.prioridad || 'Normal')}</span>
                </td>
                <td>
