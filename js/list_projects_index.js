...
function displayTopEmployeesProgress(empleados) {
	...
	// Mostrar solo top 5 para mantener compacto 
	empleados.slice(0, 5).forEach((empleado, index) => {
		const row = document.createElement('tr');
...
function displayTopProjectsProgress(proyectos) {
	...
	// Mostrar solo top 5 para mantener compacto 
	proyectos.slice(0, 5).forEach((proyecto, index) => {
		const row = document.createElement('tr');
...
function createCompactProgressBar(progress, completed, total) {
	const progressValue = parseFloat(progress) || 0;
	const progressClass = progressValue >= 75 ? 'bg-success' :
		progressValue >= 50 ? 'bg-info' :
		progressValue >= 25 ? 'bg-warning' : 'bg-danger';
...
function displayEmptyProjectsState() {
    ...
    <tr> 
      <td colspan="3" class="text-center text-muted py-2"> 
        <i class="mdi mdi-folder-off" style="font-size: 24px; opacity: 0.5;"></i> 
        <p class="mt-1 mb-0" style="font-size: 0.75rem;">Sin proyectos</p> 
      </td> 
    </tr> 
  `;
}
...
window.displayTopEmployeesProgress = displayTopEmployeesProgress;
window.displayTopProjectsProgress = displayTopProjectsProgress;
window.createCompactProgressBar = createCompactProgressBar;
window.displayEmptyEmployeesState = displayEmptyEmployeesState;
window.displayEmptyProjectsState = displayEmptyProjectsState;