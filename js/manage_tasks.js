document.addEventListener("DOMContentLoaded", function () {
  ...
  const projectPermissionNote = document.getElementById(
  ...
  function initializeUserData() {
    if (typeof window.APP_CONFIG === "undefined") {
      console.error(
        "ERROR: APP_CONFIG no está definido. Asegúrese de que el script de configuración se cargue antes de manage_tasks.js",
      );
      showAlert(
        "Error: No se pudo cargar la información del usuario. Por favor, recargue la página.",
        "danger",
      );
      return false;
    }
  ...
  if (!initializeUserData()) {
    console.error(
      "No se pudo inicializar los datos del usuario. Deteniendo la ejecución del script.",
    );
    return;
  }
    ...
    fetch("../php/get_users.php")
      ...
      .then((data) => {
        if (data.success && data.usuarios) {
          //esperamos a que se seleccione un proyecto
        }
      })
      .catch((error) => {
  ...
  function populateUserSelect(selectElement, users) {
    ...
    if (users.length === 0) {
      selectElement.innerHTML =
        '<option value="">No hay usuarios en este proyecto</option>';
  ...
  //mostrar nota informativa sobre la fecha de inicio del proyecto (sin restricción)
  function setTaskDateMinimum(projectData) {
    ...
    // Ya no establecemos un mínimo, solo mostramos información
    ...
    if (projectData && projectData.fecha_inicio) {
      ...
      // Agregar listener para mostrar advertencia si fecha es anterior
      taskDateInput.addEventListener("change", function () {
        checkTaskDateWarning(this.value, projectData.fecha_inicio);
      });
    } else {
      if (taskDateNoteDiv) {
  ...
  //crear un html para un solo elemento de tarea - FIXED: usando MDI icons para checkbox y indicador de vencido
  function createTaskElement(task, isLast = false) {
    ...
    const overdueIndicator = overdue
      ? '<span class="text-danger fw-bold ms-1" title="Tarea vencida">*</span>'
      : "";
  ...
  function handleTaskStatusChange(event) {
    ...
    const isCurrentlyChecked = icon.classList.contains(
      "mdi-checkbox-marked-circle-outline",
    );
      ...
      .then((data) => {
        if (data.success) {
          ...
          if (newStatus === "completado") {
            ...
            const overdueIndicator = taskLi.querySelector(
              ".text-danger.fw-bold",
            );
            if (overdueIndicator && overdueIndicator.textContent === "*") {
            ...
            if (dateDiv) {
              ...
              dateDiv.textContent = dateDiv.textContent.replace(
                " (Vencida)",
                "",
              );
            }
            ...
            const taskDate = editBtn
              ? editBtn.getAttribute("data-task-date")
              : null;
  ...
  function handleEditTask(event) {
    ...
    document.querySelector("#saveTaskBtn .btn-text").textContent =
      "Actualizar Tarea";
    ...
    loadProjectsForModal(() => {
      ...
      if (currentProjectData && currentProjectData.id_proyecto == taskProject) {
        ...
        // Verificar advertencia para la fecha actual
        ...
        fetchProjectDetails(taskProject, function (projectData) {
          ...
          checkTaskDateWarning(taskDate, projectData.fecha_inicio);
        });
      }
    });
    //mostrar modal
  ...
  function initializeModalEventListeners() {
    ...
    taskDateInput.addEventListener("change", function () {
      ...
      if (selectedProjectId && currentProjectData) {
        ...
        fetchProjectDetails(selectedProjectId, function (projectData) {
      ...
      if (mode !== "edit") {
        loadProjectsForModal(() => {
          if (currentProjectId) {
            ...
            //mostrar nota de fecha y actualizar permisos de asignacion basado en el proyecto actual
            setTaskDateMinimum(currentProjectData);
          ...
          fetchProjectDetails(taskProject, function (projectData) {
            ...
            if (!canAssign) {
              ...
              document.getElementById("taskAssigneeNote").innerHTML = ` 
                            ...
                            `;
              document.getElementById("taskAssigneeNote").style.display =
                "block";
            } else {
              assigneeSelect.disabled = false;
              document.getElementById("taskAssigneeNote").style.display =
    ...
    modal.addEventListener("hidden.bs.modal", function () {
      ...
      document.getElementById("addTaskModalLabel").textContent =
        "Agregar Nueva Tarea";
      document.querySelector("#saveTaskBtn .btn-text").textContent =
        "Guardar Tarea";
  ...
  function handleSaveTask() {
    ...
    if (!currentUserId) {
      showModalMessage(
        "Error: No se pudo obtener el ID del usuario. Por favor, recargue la página.",
        "danger",
      );
      return;
    }
      ...
      fetch("../php/save_task.php", {
        ...
        .then((data) => {
          ...
          if (data.success) {
            ...
            showModalMessage(
              data.message || "Error al guardar la tarea",
              "danger",
            );
          }
        })
  ...
  function updateTaskInList(
    ...
    if (dateDiv) {
      if (overdue) {
        dateDiv.classList.add("text-danger", "fw-bold");
        dateDiv.textContent = formattedDate + " (Vencida)";
      } else {
        dateDiv.classList.remove("text-danger", "fw-bold");
